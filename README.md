...
- **"that slot must not be available to others"** — this is a correctness requirement
  under concurrency, not a UI requirement. Two patients hitting _Book_ on the same slot
...
A doctor is a _resource that gets scheduled against_, not an actor. There are no doctor
...
**No Redis lock (`SETNX` + TTL).** A distributed lock in Redis is a _liveness_
optimisation, not a _safety_ guarantee. If the lock's TTL expires while the holder is
...
constraint is expressible either way — the _design_ (let the database enforce the
invariant) ports; only the _syntax_ does not.
...
practice two rows and a loop — and is _definitionally_ consistent with the working
...
re-processing. The result is stored _after_ the database write succeeds. This makes the
booking endpoint safe against the mobile-network retry — patient taps _Book_, the
...
at those ports; every value is a plain `os.Getenv` call with an inline fallback in
code, no config struct or library.
...
`main.go` loads `.env` itself on startup via `godotenv.Load()`, so no manual export
step is needed — just run the binary:
...
`godotenv.Load()` only fills in variables that aren't already set in the shell — it
never overrides one that is. That's what makes `SETUP_TYPE=migrate go run .` above
still work correctly: the shell-level `SETUP_TYPE=migrate` takes precedence over
whatever `.env` says, exactly as before. It also means the old manual-export style
(`set -a; source .env; set +a`) still works fine if you prefer it — nothing about
that habit breaks.

In production (Railway) and in CI, there is no `.env` file at all — `godotenv.Load()`
returns an error, that's logged, and the app falls through to the real environment
variables Railway/GitHub Actions inject directly. `.env` is gitignored and never
committed; the exact content above is what to put in your own local copy.
...
cannot reproduce. `go test` compiles and runs its own binary that never calls this
package's `main()`, so `godotenv.Load()` never runs for a test — the manual export
step below is still required here even though it's no longer needed for `go run .`:
...
for the reasons in §1.3);I I worked through an
...
endpoint against a live database.
...
- **Whether to pay for Railway's Hobby plan after the trial expired.** spending real money isn't something I wanted decided for
...
instructions ask for these to be called out, so the are recorded here as they are made
